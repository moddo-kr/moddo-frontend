 ...
 * @returns 각 인원별 분배 금액 배열
 */
const distributeAmount = (
  totalAmount: number,
  numberOfMembers: number
): number[] => {
  if (numberOfMembers <= 0) return [0];

  const baseAmount = Math.floor(totalAmount / numberOfMembers); // 기본 분배 금액
  const remainder = totalAmount % numberOfMembers; // 나누어 떨어지지 ㅇ낳는 나머지 금액

  // 첫 번째 사람에게 나머지 금액을 추가한다.
  return Array.from({ length: numberOfMembers }, (_, index) =>
